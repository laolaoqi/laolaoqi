// ===================================================================
// CryptoInvestment — 主流币 vs 空气币永续合约 投资看板
          ...
          <div className="flex items-center h-14 gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#8899aa] -mt-0.5">主流币 vs 空气币永续合约</p>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[rgba(0,212,255,0.06)]">
                      <th className="text-left px-4 py-2.5 text-xs text-[#8899aa] font-medium">#</th>
                      <th className="text-left px-4 py-2.5 text-xs text-[#8899aa] font-medium">币种</th>
                      <th className="text-right px-4 py-2.5 text-xs text-[#8899aa] font-medium">价格 (USD)</th>
                      <th className="text-right px-4 py-2.5 text-xs text-[#8899aa] font-medium">24h涨跌</th>
                      <th className="text-right px-4 py-2.5 text-xs text-[#8899aa] font-medium hidden sm:table-cell">市值</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.mainstream.length === 0 ? (
                      <tr><td colSpan={5} className="text-center py-8 text-[#8899aa]">暂无数据</td></tr>
                    ) : (
                      data.mainstream.map((coin, i) => {
                        ...
                        return (
                          <tr key={coin.symbol} className="border-b border-[rgba(0,212,255,0.04)] hover:bg-[rgba(0,212,255,0.03)] transition-colors">
                            <td className="px-4 py-3 text-xs text-[#667788] font-mono">{i + 1}</td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-bold text-white">{coin.name}</span>
                                <span className="text-xs text-[#667788]">{coin.symbol}</span>
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-sm font-bold font-mono ${
                            ...
                            <td className="px-4 py-3 text-right text-xs text-[#8899aa] font-mono hidden sm:table-cell">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[rgba(255,107,0,0.06)]">
                      <th className="text-left px-4 py-2.5 text-xs text-[#8899aa] font-medium">#</th>
                      <th className="text-left px-4 py-2.5 text-xs text-[#8899aa] font-medium">合约</th>
                      <th className="text-right px-4 py-2.5 text-xs text-[#8899aa] font-medium">价格 (USD)</th>
                      <th className="text-right px-4 py-2.5 text-xs text-[#8899aa] font-medium">24h涨跌</th>
                      <th className="text-right px-4 py-2.5 text-xs text-[#8899aa] font-medium hidden sm:table-cell">24h成交额</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.meme.length === 0 ? (
                      ...
                      data.meme.map((coin, i) => {
                        const isUp = coin.change24h >= 0;
                        return (
                          <tr key={coin.symbol} className="border-b border-[rgba(255,107,0,0.04)] hover:bg-[rgba(255,107,0,0.03)] transition-colors">
                            <td className="px-4 py-3 text-xs text-[#667788] font-mono">{i + 1}</td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-bold text-white">{coin.name}</span>
